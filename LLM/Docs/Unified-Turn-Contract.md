...
7. **Documentation open / navigation (DD-DOC-001 / TD-173):** Query docs via `search_project_docs` / library `search_*_docs` (snippets include `doc_uri`). Open GUI Help / ClDesc / markdown Docs with `open_help` / `open_class_docs` / `open_documentation`. Multi-hit answers list markdown links; chat clicks `nmsdk-doc|help|class` via presentation sink. See [Agent-Documentation.md](Agent-Documentation.md).
