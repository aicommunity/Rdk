...
6. **Capability Risk Gate (DD-CAP-001 / TD-172):** when the Cortex model is too weak for the turn, fuse pre/mid signals (weak profile, complex write goals, AoC recoveries, tool errors) into a risk score. If above `NMSDK_LLM_CAPABILITY_RISK_TAU` (default 0.55): **pre-escalate** or **one cascade retry** via `model_override` to a stronger catalog model (`ollama-thinking`); else **abstain** with `error.model_too_weak_for_request` and **do not** use `query_soft_fallback` under high risk. Audits: `capability_pre_escalate`, `capability_cascade_retry`, `capability_risk_abstain`. See [Weak-Model-Mitigation.md](Weak-Model-Mitigation.md). Disable: `NMSDK_LLM_CAPABILITY_RISK_DISABLE=1`.
7. **Documentation open / navigation (DD-DOC-001 / TD-173, DD-DOC-002 / TD-174):** Query docs via `search_project_docs` / library `search_*_docs` (snippets include `doc_uri`; prefetch lines may include `doc_uri`). Catalog inventory → `list_*_component_classes` first. Open GUI Help / ClDesc / markdown Docs with `open_help` / `open_class_docs` / `open_documentation`. Multi-hit answers list markdown links; if the user asked for links and prose lacks `nmsdk-*`, orchestrator appends a deterministic footer from tool_trace. Chat clicks `nmsdk-doc|help|class` via presentation sink. See [Agent-Documentation.md](Agent-Documentation.md).
8. **Live analogous connect (DD-CONN-002 / TD-152):** when ConnectPlan builds with `analogous_ref_token`, execute (or confirm) even under HintOnly — ReAct essay is not the primary path.
