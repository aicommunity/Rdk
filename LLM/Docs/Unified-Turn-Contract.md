...
5. **Act-or-Clarify (DD-ACT-001 / TD-153):** when tools were offered and the goal is actionable (Mutate/Plan writes, connect/add/lifecycle detectors, **or Query/Explain without successful read evidence yet**), an empty `tool_calls` round must not finalize as free prose — recovery → tool / `ask_user` / `NO_SUITABLE_TOOL`. After a **successful read-only tool trace** (e.g. `get_net_snapshot` ok, no writes), prose is allowed for Query/Explain and for Mutate false-positives — **except** when a capability pack sets `force_include_write` (e.g. `project_description`): then prose stays blocked until `update_configuration` succeeds (chat 17-49-10). **Failed-only** tool traces do **not** unlock prose. Capability packs may set `force_include_write` so Write tools stay in the API under Query. Does not force API `tool_choice` under thinking (DD-THINK-003); enforcement is post-completion in the orchestrator loop.
