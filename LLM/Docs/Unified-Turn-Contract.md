...
5. **Act-or-Clarify (DD-ACT-001 / TD-153):** when tools were offered and the goal is actionable (Mutate/Plan writes, connect/add/lifecycle detectors, **or Query/Explain without tool evidence yet**), an empty `tool_calls` round must not finalize as free prose — recovery → tool / `ask_user` / `NO_SUITABLE_TOOL`. After **any tool result in the current turn**, Query/Explain may synthesize prose from tool results (do not map exhaust to «не найдено действие» when the model already answered). Does not force API `tool_choice` under thinking (DD-THINK-003); enforcement is post-completion in the orchestrator loop.
