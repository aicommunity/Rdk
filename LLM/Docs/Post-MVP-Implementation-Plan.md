...
|----|-------|----------------|----------|
| TD-041 | Manual GUI write-tools walkthrough | done (`Test_LLM_TD041WalkthroughSim`; optional human spot-check) | P0 — QA |
...
| TD-036 | Embedding-based dynamic tool routing | done (env-gated `ULLMDynamicToolRouter`) | P3 |
| TD-031 | YAML knowledge manifest | done (`UYamlKnowledgeCatalog`) | P3 |
| TD-034 | Function-level source chunking (ctags) | done (`UDocCtagsChunker`) | P3 |
| TD-138 | Unify remaining writes off `MModel_*` | **deferred indefinite** (umbrella Track 1, 2026-07-27) | P2 |
...
**Exit:** ✅ Closed — `Test_LLM_TD041WalkthroughSim` covers HITL reject + auto-apply retry; optional human checklist remains in [Chat-22-15-GUI-Walkthrough.md](Chat-22-15-GUI-Walkthrough.md) / Application-Commands Manual E2E (not scheduled).
...
| TD-041 | ✅ Walkthrough sim done (`Test_LLM_TD041WalkthroughSim`); optional human spot-check deferred |
| TD-035 | UI flag + tests (done) |
| TD-031/034/036 | Each `done` or `cancelled` with TECH-DEBT resolution |
| TD-138 | **deferred indefinite** (2026-07-27 umbrella Track 1) — not blocking Post-MVP close |
...
- Restored truncated Post-MVP / TECH-DEBT / Post-Audit docs after bilingual cleanup regression.
- Track 1 (TD-138 write-path unification, manual GUI QA expansion, scenario growth) **shelved indefinitely**; see TECH-DEBT Deferred decisions + TD-138 Notes.
