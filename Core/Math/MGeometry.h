...
MMatrix<T,4,4> CalcObjectPositionMatrix(const MVector<T,6> &anglesANDshifts)
{
	...
	cos_gamma       = cos(anglesANDshifts(0));
	sin_gamma       = sin(anglesANDshifts(0));
	cos_beta       = cos(anglesANDshifts(1));
	sin_beta       = sin(anglesANDshifts(1));
	cos_alpha       = cos(anglesANDshifts(2));
	sin_alpha       = sin(anglesANDshifts(2));
	...
	res(0,3)=anglesANDshifts(3);
	res(1,3)=anglesANDshifts(4);
	res(2,3)=anglesANDshifts(5);
...
void CalcObjectAnglesAndShifts(const MMatrix<T,4,4> &ExtMat, MVector<T,6> &anglesANDshifts)
{
	...
	anglesANDshifts(1) = -asin( ExtMat(0,2));        // Вычисления угла вращения вокруг оси Y 
    C           =  cos( anglesANDshifts(1) );
    ...
    if ( fabs( C ) > 0.005 )          // "Шарнирный замок" (Gimball lock)? 
      ...
      anglesANDshifts(0)  = atan2( trY, trX );

      trX      =  ExtMat(0,0) / C;            // Получаем угол вращения вокруг оси  Z 
      trY      =  -ExtMat(0,1) / C;

      anglesANDshifts(2)  = atan2( trY, trX );
      }
    else                                 // Имеет место "Шарнирный замок" (Gimball lock) 
      {
      anglesANDshifts(0)  = 0;                      // Угол вращения вокруг оси X приравниваем к нулю 
	...
	anglesANDshifts(3)=ExtMat(0,3);
	anglesANDshifts(4)=ExtMat(1,3);
	anglesANDshifts(5)=ExtMat(2,3);
...
void CalcObjectAnglesAndShiftsD(const MDMatrix<T> &ExtMat, MDMatrix<T> &anglesANDshifts)
{
	...
	AnS(1) = -asin( ExtMat(0,2));        // Вычисления угла вращения вокруг оси Y 
    C           =  cos( AnS(1) );
    ...
    if ( fabs( C ) > 0.005 )          // "Шарнирный замок" (Gimball lock)? 
      ...
      {
      AnS(0)  = 0;                      // Угол вращения вокруг оси X приравниваем к нулю 
