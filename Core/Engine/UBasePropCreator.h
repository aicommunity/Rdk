...
public:
    ...
    template<typename T>
    static std::shared_ptr<UIProperty> CreateModernProperty(const std::string& name, T&& value) {
        return std::make_shared<UVBaseDataProperty<T>>(new T(std::forward<T>(value)));
    }
};
