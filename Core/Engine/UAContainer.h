...
bool Save(UVariableData *values);
...
bool Load(UVariableData *values, UAContainerStorage *storage=0, bool paramsonly=false);
                     */
...
// Если флаг paramsonly == true то только обновляет данные параметров
// для компонента и всех его уже существующих дочерних компонент
//bool Load(RDK::UClassRegistry &buffer, UAContainerStorage *storage=0, bool paramsonly=false);
