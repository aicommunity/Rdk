...
```xml
<ProjectAutoSaveModelTimeInterval>30</ProjectAutoSaveModelTimeInterval>
```

При интервале `30` консоль проверяет время модели раз в 500 мс и сохраняет проект, когда любой канал проходит очередной интервал. `SaveProject()` записывает конфигурацию проекта и модель с параметрами по путям, заданным в конфиге. Для `NNeuronLearner` массивы `DendriteLength`, `NumSynapse` и текущие статусы находятся в `Parameters_00.xml`, а `Model_00.xml` описывает состав компонентов и соединения; для полного снимка структуры нужны оба файла. Файлы состояний записываются, только если включён `ProjectAutoSaveStateFlag`. Для экспериментов используйте копию проекта: периодическое сохранение перезаписывает указанные в ней файлы.

Запуск без `-x` оставляет расчёт работающим, а снимки появляются по ходу вычисления:

```powershell
& '.\NeuroModelerConsole.exe' -c '.\project.ini' -s
```

Можно совместить автосохранение с конечным пределом, `-x` и `-S`; тогда будут промежуточные снимки и завершающее сохранение после остановки. Время в логе относится к каналу, который первым достиг порога; сама запись происходит после срабатывания 500-миллисекундного таймера и получения доступа к движку. Из-за шага расчёта и ожидания блокировки запись может отставать от порога на несколько модельных секунд: проверка с интервалом 2 секунды записала снимки при 3, 6, 9 и 12 секундах. Интервал задаётся в целых секундах и служит порогом, а не гарантией точности до секунды.

`UApplication::SaveProject()` сейчас возвращает `true` даже если отдельная запись файла не удалась; сообщения об ошибках появляются в журнале приложения. Поэтому факт обновления файлов следует подтверждать по их содержимому и времени изменения. Включение `ProjectAutoSaveStateFlag` сохраняет дополнительные данные состояния, однако само по себе не гарантирует точное продолжение прерванного обучения.
