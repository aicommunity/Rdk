...
stateDiagram-v2
    ...
    Initialization --> LoadingLibraries: Загрузка библиотек
    LoadingLibraries --> BuildingStorage: Сборка хранилища
    BuildingStorage --> InitializingLogging: Initialization логирования
    InitializingLogging --> CreatingComponents: Creation компонентов
    CreatingComponents --> Ready: Готово к работе
    
    Ready --> OpeningProject: Открытие проекта
    OpeningProject --> LoadingProject: Загрузка проекта
    LoadingProject --> CreatingChannels: Creation каналов
    CreatingChannels --> BuildingModels: Сборка моделей
    BuildingModels --> ProjectReady: Проект готов
    ...
    Running --> UpdatingInterface: Обновление интерфейса
    UpdatingInterface --> ProcessingRPC: Processing RPC команд
    ProcessingRPC --> Running: Продолжение расчета
...
flowchart LR
    ...
    Engine -->|uses| Graphics
    Engine -->|uses| Serialize
    Engine -->|uses| System
    Engine -->|uses| Math
    Engine -->|uses| Utilities
    
    Graphics -->|uses| System
    Graphics -->|uses| Utilities
    
    Serialize -->|uses| System
    Serialize -->|uses| Utilities
    
    Math -->|uses| System
    Math -->|uses| Utilities
    
    System -->|uses| Utilities
```
